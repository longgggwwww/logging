...
export const shouldSendNotification = (logData: LogData): boolean => {
  ...
  if (!CONFIG.discord.filter.enabled) {
    return true;
  }
  ...
  if (
    !logData.type ||
    !CONFIG.discord.filter.criticalTypes.includes(logData.type)
  ) {
    console.log(
  ...
  if (responseCode < CONFIG.discord.filter.minSeverityCode) {
    console.log(
      `🔕 Filtered: Response code ${responseCode} < ${CONFIG.discord.filter.minSeverityCode} (not severe enough)`
    );
    return false;
  }
