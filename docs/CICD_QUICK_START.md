...
└── trigger-rebuild.sh    # Trigger service rebuild (NEW)
...
services/*/. rebuild       # Rebuild flags for each service (NEW)
web-app/.rebuild          # Rebuild flag for web-app (NEW)
```
...
1. **VPS_SSH_PRIVATE_KEY**: Nội dung của `~/.ssh/github-actions` (Ed25519)
2. **VPS_HOST**: IP hoặc domain của VPS (ví dụ: `192.168.1.100`)
3. **VPS_USER**: Username trên VPS (ví dụ: `deploy` hoặc `ubuntu`)

**Optional** (cho proxy jump):

4. **VPS_BASTION_HOST**: Bastion host (ví dụ: `bastion.example.com`)
5. **VPS_BASTION_USER**: Bastion username (ví dụ: `jump-user`)

> Nếu VPS nằm sau bastion, thêm 2 secrets optional. Workflow tự động detect.
...
**Scenario 4: Trigger rebuild mà không sửa code (NEW)**
```
Action: ./scripts/trigger-rebuild.sh api
→ Build: api only (2-3 min)
→ Deploy: api only (1 min)
→ Total: ~3-4 min
Use case: Update dependencies, force redeploy
```

**Scenario 5: Rebuild tất cả services (NEW)**
```
Action: ./scripts/trigger-rebuild.sh all
→ Build: All services (parallel, ~5-8 min)
→ Deploy: All services (2-3 min)
→ Total: ~7-11 min
Use case: Infrastructure changes, reset deployment
```
...
Sử dụng khi cần rebuild service mà không thay đổi code:
...
📖 **Chi tiết**: [docs/REBUILD_TRIGGER.md](./REBUILD_TRIGGER.md)

## �📖 Documentation

- Chi tiết: [docs/CICD_SETUP.md](./CICD_SETUP.md)
- Rebuild Trigger: [docs/REBUILD_TRIGGER.md](./REBUILD_TRIGGER.md)
