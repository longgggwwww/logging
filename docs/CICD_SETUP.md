...
```bash
# Tạo SSH key mới (Ed25519 - recommended)
...
> **Note**: Ed25519 nhanh hơn, an toàn hơn và nhỏ gọn hơn RSA 4096-bit.
...
1. **VPS_SSH_PRIVATE_KEY**
   ...
   ```
   Paste toàn bộ nội dung (bao gồm cả `-----BEGIN OPENSSH PRIVATE KEY-----`)
...
4. **VPS_BASTION_HOST**
   ```
   # IP hoặc domain của bastion/jump host
   Ví dụ: bastion.example.com hoặc 10.0.0.1
   ```

5. **VPS_BASTION_USER**
   ```
   # Username trên bastion server
   Ví dụ: jump-user
   ```

> **Note về Proxy Jump**: Nếu VPS của bạn nằm trong private network và chỉ có thể access qua bastion host, thêm 2 secrets optional trên. Workflow sẽ tự động detect và setup SSH proxy jump.
...
Đôi khi bạn cần rebuild service mà không thay đổi code:
- Sau khi update dependencies
- Force redeploy service
- Test CI/CD pipeline
- Sau khi thay đổi infrastructure
...
Khi file này thay đổi, GitHub Actions sẽ tự động phát hiện và rebuild service đó.

📖 **Chi tiết**: Xem [REBUILD_TRIGGER.md](./REBUILD_TRIGGER.md)
