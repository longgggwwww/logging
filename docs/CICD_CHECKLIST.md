...
- [ ] **Generate SSH Keys**
  ...
  # Script sẽ tạo Ed25519 key pair
  ```
...
- [ ] **Test SSH Connection**
  ```bash
  # From local machine (direct)
  ssh -i ~/.ssh/github-actions user@your-vps
  
  # Via bastion (if using proxy jump)
  ssh -J bastion-user@bastion-host -i ~/.ssh/github-actions user@your-vps
  ```
...
- [ ] **Add Repository Secrets**
  ...
  **Required Secrets:**
  ...
  - [ ] **VPS_SSH_PRIVATE_KEY**
    ```bash
    # Copy entire content of Ed25519 private key
    cat ~/.ssh/github-actions
    # Include -----BEGIN OPENSSH PRIVATE KEY----- lines
    ```
  ...
  **Optional Secrets (for proxy jump):**
  
  - [ ] **VPS_BASTION_HOST** (if using bastion)
    ```bash
    # Bastion host IP or domain
    # Example: bastion.example.com
    ```
  
  - [ ] **VPS_BASTION_USER** (if using bastion)
    ```bash
    # Username on bastion server
    # Example: jump-user
    ```
...
- [ ] **Verify Secrets**
  ...
  - [ ] VPS_BASTION_* set if using proxy jump
...
## 🔁 Rebuild Triggers (NEW)

### ☐ Test Rebuild Feature

- [ ] **Test single service rebuild**
  ```bash
  # Trigger rebuild for API
  ./scripts/trigger-rebuild.sh api
  
  # Commit and push
  git add .
  git commit -m "chore: test api rebuild"
  git push
  ```
...
- [ ] **Test all services rebuild**
  ...
  # Commit and push
  git add .
  git commit -m "chore: test rebuild all"
  git push
  ```
  - [ ] All flags toggled
  - [ ] GitHub Actions triggered
...
- [ ] **Understand rebuild flags**
  - [ ] Know where rebuild flags are located
  - [ ] Understand flag toggle (0 ↔ 1)
  - [ ] Read [REBUILD_TRIGGER.md](./REBUILD_TRIGGER.md)
